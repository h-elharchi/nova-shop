...
const ALL_STATUSES: { value: OrderStatus | ''; labelKey: string }[] = [
  { value: '',            labelKey: 'order.filter_all'         },
  { value: 'new',         labelKey: 'order.status_new'         },
  { value: 'assigned',    labelKey: 'order.status_assigned'    },
  { value: 'contacted',   labelKey: 'order.status_contacted'   },
  { value: 'unreachable', labelKey: 'order.status_unreachable' },
  { value: 'callback',    labelKey: 'order.status_callback'    },
  { value: 'confirmed',   labelKey: 'order.status_confirmed'   },
  { value: 'processing',  labelKey: 'order.status_processing'  },
  { value: 'shipped',     labelKey: 'order.status_shipped'     },
  { value: 'delivered',   labelKey: 'order.status_delivered'   },
  { value: 'returned',    labelKey: 'order.status_returned'    },
  { value: 'cancelled',   labelKey: 'order.status_cancelled'   },
  { value: 'on_hold',     labelKey: 'order.status_on_hold'     },
]

const ALL_CHANNELS: { value: OrderChannel | ''; labelKey: string }[] = [
  { value: '',          labelKey: 'order.filter_all_channels' },
  { value: 'site',      labelKey: 'order.channel_site'        },
  { value: 'whatsapp',  labelKey: 'order.channel_whatsapp'    },
  { value: 'email',     labelKey: 'order.channel_email'       },
  { value: 'chat',      labelKey: 'order.channel_chat'        },
  { value: 'phone',     labelKey: 'order.channel_phone'       },
]
...
function computeDateRange(period: DatePeriod, customFrom: string, customTo: string) {
  const now   = new Date()
  ...
  switch (period) {
    ...
    case 'yesterday': {
      ...
      const d = localDateStr(y); return { dateFrom: d, dateTo: d }
    }
    case '7days': {
...
function formatPhone(phone: string): string {
  ...
  if (c.startsWith('00212')) return '+' + c.slice(2)
  return c
}
...
export function AdminOrdersPage() {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const filename   = `nova-shop-commandes-${today}.xlsx`
      exportOrdersToExcel(
        filteredOrders,
        {
  ...
  }, [exporting, filteredOrders, lang, t])
  ...
  return (
    <AdminLayout>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex flex-col items-end gap-1">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-sm font-semibold rounded-xl transition-colors shadow-sm"
            >
              <Download className="w-4 h-4" />
              {exporting ? t('order.exporting') : t('order.export_excel')}
            </button>
            {exportMsg && (
              <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400">{exportMsg}</span>
            )}
          </div>
        </div>
      </div>
      ...
      {/* Filtres */}
      <div className="bg-white dark:bg-dark-card rounded-2xl shadow-sm border border-gray-100 dark:border-dark-border p-4 mb-4 space-y-4 transition-colors">
        <div className="relative">
          ...
          <input
            ...
            className="w-full border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-xl pl-9 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition placeholder:text-gray-400"
          />
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          ...
          <div>
            ...
            >
              {ALL_STATUSES.map(s => <option key={s.value} value={s.value}>{t(s.labelKey)}</option>)}
            </select>
          </div>

          {/* Canal */}
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{t('order.filter_channel')}</label>
            <select
              value={channelFilter}
              onChange={e => setChannelFilter(e.target.value as OrderChannel | '')}
              className="w-full border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
            >
              {ALL_CHANNELS.map(c => <option key={c.value} value={c.value}>{t(c.labelKey)}</option>)}
            </select>
          </div>

          {/* Produit */}
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{t('order.filter_product')}</label>
            <select
              value={productFilter}
          ...
          {/* Période */}
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{t('order.filter_period')}</label>
            <select
              value={datePeriod}
              onChange={e => { setDatePeriod(e.target.value as DatePeriod); setCustomFrom(''); setCustomTo('') }}
              className="w-full border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
            >
              {DATE_PERIODS.map(p => <option key={p.value} value={p.value}>{t(p.labelKey)}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 pt-1">
          ...
          {!loading && (
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {filteredOrders.length === 0 ? t('order.no_orders_found') : `${filteredOrders.length} ${t('order.orders_count')}`}
            </span>
          )}
        </div>
      </div>
      ...
      {!loading && error && <div className="text-center py-10 text-red-500 text-sm">{error}</div>}
      ...
      {!loading && !error && paginatedOrders.length > 0 && (
        <div className="space-y-3">
          {paginatedOrders.map(order => {
            ...
            const waMsg = encodeURIComponent(`Bonjour ${order.customer_first_name}, concernant votre commande : ${order.product_name}.`)
            const img   = order.product?.images?.[0]?.image_url

            return (
              <div key={order.id} className="bg-white dark:bg-dark-card rounded-2xl shadow-sm border border-gray-100 dark:border-dark-border p-4 transition-colors">
                <div className="flex items-start gap-4">
                  <div className="shrink-0">
                    {img ? (
                      <img src={img} alt={order.product_name} className="w-14 h-14 rounded-xl object-cover" />
                    ) : (
                      <div className="w-14 h-14 bg-gray-100 dark:bg-gray-700 rounded-xl flex items-center justify-center text-2xl">🛍️</div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {order.callback_at && (
                      <p className="text-xs text-purple-600 dark:text-purple-400 mt-0.5">
                        📅 {new Date(order.callback_at).toLocaleString('fr-MA')}
                      </p>
                    )}
      ...
      {!loading && totalPages > 1 && (
        <div className="flex items-center justify-center gap-3 mt-6">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
            className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-40 transition-colors">
            {t('order.prev_page')}
          </button>
